...
侧栏“透传报文解析”支持粘贴一条或用双 `&&` 分隔的多条 Hex 报文。程序会先删除空白并逐段处理；单个 `&`、非法 Hex 或单条校验失败不会中止其他片段。

当前确定性代码支持带 `68 + ASCII 序列号 + 68` 外层及无外层的 Modbus RTU，并保留私有功能码原始 PDU；也支持带一个或多个 `FE` 唤醒字节的 DL/T 645-2007 与 CJ/T 188-2018 完整标准帧结构、长度、结束符和加和校验。两个标准候选均不成立或同时成立时保持协议未知。没有上传资料时不会调用 AI，也不会补造参量名称。

AWT 模板模式会读取 Rust 文件选择器授权的标准 12 列 CSV，按地址关联内置参量目录并生成换算值。寄存器说明书模式复用现有文档提取、分块、AI CSV、一次结构纠错和失败块二分流程，在内存中生成仅服务于本次解析的临时 RegisterMap，不保存中间 AWT 文件。AI 或资料提取失败时保留输入，用户可清除资料后按无资料模式重新解析。
